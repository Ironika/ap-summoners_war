...
import org.ap.summonerwar.helpers.DoBuildHelper;
...
public class BuildServlet extends APServletBase {
	...
	public Response postBuild(@Context SecurityContext sc, BuildBean buildBean) {
		try {
			...
			data.setRunesLvl(buildBean.runesLvl);
			data.setName(buildBean.name);
			data.setState(buildBean.state);
			data.setRunesStars(buildBean.runesStars);
	...
	@Consumes({MediaType.APPLICATION_JSON})
	@RolesAllowed("user")
	public Response postBuildDo(@Context SecurityContext sc, DoBuildBean doBuildBean) {
		try {
			Object bean = DoBuildHelper.postDoBuild(sc, doBuildBean);
			return Response.status(Status.OK).entity(bean).build();

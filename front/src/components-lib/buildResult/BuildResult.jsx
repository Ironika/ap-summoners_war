...
class BuildResults extends React.Component {
	...
	constructor(props) {
		super(props);
		this.state = { showResult: false, loaded: false}
	}
	...
	onClickShow(id) {
    	this.setState({showResult: !this.state.showResult, loaded: true})
    }

	render() {
		let buildResult = this.props.buildResult
		...
		return (
			<div className={"sm-build-result"} key={buildResult.id}>
				...
				<div className={"sm-build-team-results " + (this.state.showResult ? "" : "sm-hide")} >
					{this._buildTeamResults(buildResult)}
				</div>
				<hr/>
			</div>
		)
